/**
 * ML API client for confusion probability model.
 */
...
export async function getInitialState(priorStrength = 1.0): Promise<ConfusionState> {
  try {
    ...
    // Return default state with uniform prior
...
export function loadConfusionState(): ConfusionState | null {
  try {
    const stored = localStorage.getItem(CONFUSION_STATE_KEY);
    if (stored) {
      return JSON.parse(stored);
    }
  } catch (e) {
    console.error('Failed to load confusion state from localStorage:', e);
  }
  return null;
}

export function saveConfusionState(state: ConfusionState): void {
  try {
    localStorage.setItem(CONFUSION_STATE_KEY, JSON.stringify(state));
  } catch (e) {
    console.error('Failed to save confusion state to localStorage:', e);
  }
}
...
export async function getSequenceErrorProb(
  ...
  // For now, use the first tone in sequence as the "problem"
  // This is a simplification - could be extended to consider all tones
