...
interface VowelGridProps {
  correctSequence: VowelId[];
  distractorSequences: VowelId[][];
  onSelect: (selectedSequence: VowelId[], isCorrect: boolean) => void;
  disabled?: boolean;
}

export function VowelGrid({
  correctSequence,
  distractorSequences,
  onSelect,
  disabled = false,
  ...
  const options: VowelOption[] = useMemo(() => {
  ...
  const handleSelect = (option: VowelOption) => {
  ...
  const getButtonStyle = (option: VowelOption) => {
    if (!showResult) {
      return 'border-gray-200 hover:border-purple-400 hover:shadow-lg bg-white';
    }
        ...
        return (
          ...
          >
            ...
            <div className="text-4xl font-bold text-purple-600 tracking-widest">
              {formatVowelSequence(option.sequence)}
            </div>
